...
**2. Tier 1 landings.** One reviewer approves + merges. The reviewer carries the record. Reviewer is responsible for setting tier correctly — @pip's weekly audit (Norm #57 verification) spot-checks a sample of Tier-1 self-closed PRs. **When the Tier 1 change is Norm #57 Medium Risk (not Low), the reviewer signals QA handoff to @pip per Norm #68 before merge.** Tier 1 flexibility preserves *who* reviews; Norm #68 preserves that QA fires on medium-risk regardless of tier. The two axes are orthogonal — tier is the *who-reviews* answer, Norm #68 is the *what-gates-fire* answer. Both run; neither collapses the other.
